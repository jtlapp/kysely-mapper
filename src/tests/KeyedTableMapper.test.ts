...
describe('keyed table mapper using a single-column tuple key', () => {
  ...
  it('inserts, selects, updates, and deletes objects by key', async () => {
    // Add users
    const id0 = (await explicitKeyedMapper.insert(USERS[0])).id;
    const id1 = (await explicitKeyedMapper.insert(USERS[1])).id;
  ...
  it('updates returning default key by default with default key', async () => {
    ...
    const id1 = (await defaultKeyMapper.insert(USERS[1])).id;
  ...
  it("provides a default key of 'id'", async () => {
    ...
    await defaultIdMapper.insert(USERS[0]);
    const id1 = (await defaultIdMapper.insert(USERS[1])).id;
  ...
  it('allows for returning other columns with the key', async () => {
    ...
    const insertReturn1 = await idAndHandleMapper.insert(USERS[0]);
    expect(insertReturn1).toEqual({
      id: 1,
      handle: USERS[0].handle,
    });

    const insertReturn2 = await idAndHandleMapper.insert(USERS[1]);
  ...
  it('allows for returning all columns', async () => {
    ...
    const insertReturn1 = await allColumnsMapper.insert(USERS[0]);
    expect(insertReturn1).toEqual({
      id: 1,
      handle: USERS[0].handle,
      name: USERS[0].name,
    ...
    const insertReturn2 = await allColumnsMapper.insert(USERS[1]);
    expect(insertReturn2).toEqual({
      id: 2,
      handle: USERS[1].handle,
      name: USERS[1].name,
  ...
  it('transforms inputs and outputs', async () => {
    ...
    const insertReturn1 = await testTransformMapper.insert(insertedUser1);
  ...
  it('returns nothing when there are no return columns', async () => {
    ...
    const insertReturn1 = await noReturnMapper.insert(USERS[0]);
